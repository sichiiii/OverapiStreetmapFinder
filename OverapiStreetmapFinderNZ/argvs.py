...
class Overpy_map():
    ...
    def get_apartments(self):
        try:
            state_arr = ['NZ-AUK', 'NZ-BOP', 'NZ-CAN', 'NZ-CIT', 'NZ-GIS', 'NZ-HKB', 'NZ-MWT', 'NZ-MBH', 'NZ-NSN', 'NZ-NTL', 'NZ-OTA', 'NZ-STL', 'NZ-TKI', 'NZ-TAS', 'NZ-WKO', 'NZ-WGN', 'NZ-WTC']
...
class Excel():
    ...
    def insert_data(self, aparts, filename):
        try:
            wb = Workbook(os.path.dirname(os.path.abspath(__file__)) + '/results/' + 'NZ' + '/' +f'{filename}.xlsx')  #добавить создание нового файла
        ...
        try:
            ...
            for i, row in enumerate(aparts): 
                if 'addr:street' in row.keys():
                    ...
                    worksheet.write(counter+1, 5, 'New Zealand') 
